...
testcase_1 () {
	...
	perl $SQLTeXLocation/SQLTeX.pl --use-local-config --username $USR --password $PWD --quiet --null-allowed --filename-extend _{P1} sqltex-tc1 $1
	check_result 1 $1
}
...
testcase_2 () {
	...
	perl $SQLTeXLocation/SQLTeX.pl --use-local-config --user $USR --password $PWD --quiet --update sqltex-tc2.tex
	check_result 2 stx
}
...
testcase_3 () {
	...
	perl $SQLTeXLocation/SQLTeX.pl --use-local-config --user $USR --password $PWD --quiet sqltex-tc3.tex
	check_result 3 stx
}
...
testcase_4 () {
	...
	perl $SQLTeXLocation/SQLTeX.pl --use-local-config --user $USR --password $PWD --quiet --replace $SQLTeXTestLocation/sqltex-tc4_rpl.dat --filename-extend _nl sqltex-tc4.tex
	check_result 4 nl
}
...
testcase_5 () {
	...
	perl $SQLTeXLocation/SQLTeX.pl --use-local-config --user $USR --password $PWD --quiet --multidoc-numbered sqltex-tc5.tex
	...
	perl $SQLTeXLocation/SQLTeX.pl --use-local-config --user $USR --password $PWD --quiet --multidoc-named sqltex-tc5.tex
